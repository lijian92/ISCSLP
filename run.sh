...
local/Ti_prep_lm.sh || exit 1;

local/Ti_prep_dict.sh || exit 1;
...
# DNN without pre-training
<< com
#quick_ali
steps/align_fmllr.sh --nj $train_nj --cmd "$train_cmd" data/train data/lang exp/tri4 exp/tri4_ali || exit 1;

#quick_ali_cv
steps/align_fmllr.sh --nj $train_nj --cmd "$train_cmd" data/dev data/lang exp/tri4 exp/tri4_ali_cv || exit 1;


#train dnn model
local/nnet1/run_dnn.sh --stage 0 --nj $train_nj  exp/tri4 exp/tri4_ali exp/tri4_ali_cv || exit 1;  
com
